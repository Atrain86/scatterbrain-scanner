...
export async function compressReceiptImage(file: File): Promise<File> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // Cap at 1200px on longest side for OCR quality
      const maxDim = 1200;
...
// Aggressively compress for IndexedDB storage — smaller file size, still viewable
export async function compressForStorage(file: File): Promise<File> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      let { width, height } = img;
      // 800px max, 50% quality — keeps base64 under ~200KB for long receipts
      const maxDim = 800;
      if (width > maxDim || height > maxDim) {
        const ratio = Math.min(maxDim / width, maxDim / height);
        width  = Math.round(width  * ratio);
